 ...
 * 
 */
public class ApplicationEntry<T extends Serializable> implements Map<ApplicationKey, ApplicationEntry<?>> {
  @Override
  public int size() {
  ...
  private Map<ApplicationKey, ApplicationEntry<?>> getRepository() {
    if (null == _repository)
      _repository = new ConcurrentHashMap<ApplicationKey, ApplicationEntry<?>>();
  ...
  private ConcurrentMap<ApplicationKey, ApplicationEntry<?>> _repository;

  public ApplicationEntry(final T value) {
    _value = value;
  }
